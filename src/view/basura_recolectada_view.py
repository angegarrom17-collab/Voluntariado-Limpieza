...
class BasuraRecolectadaVista(tk.Frame):
    ...
    def _build_topbar(self):
        ...
        tk.Label(bloque_texto, text="Basura Recolectada", font=("Segoe UI", 13, "bold"), bg=COLOR_TOPBAR,
                 fg="#E8F5E9").pack(anchor="w")
        tk.Label(bloque_texto, text="Sistema de monitoreo costero  ·  Modulo de residuos", font=("Segoe UI", 9),
                 bg=COLOR_TOPBAR, fg="#C8E6C9").pack(anchor="w")
        bloque_btns = tk.Frame(topbar, bg=COLOR_TOPBAR)
        bloque_btns.pack(side="right", padx=20)
        tk.Button(bloque_btns, text="Registrar", font=("Segoe UI", 10, "bold"), bg=COLOR_ACENTO, fg="white",
                  activebackground="#388E3C", activeforeground="white", bd=0, padx=18, pady=6, cursor="hand2",
                  command=self._registrar).pack(side="left", padx=(0, 8))
        tk.Button(bloque_btns, text="Limpiar", font=("Segoe UI", 10), bg=COLOR_TOPBAR, fg="#E8F5E9",
                  activebackground=COLOR_TOPBAR2, activeforeground="white", bd=1, relief="solid", padx=14, pady=6,
                  cursor="hand2", command=self._limpiar_campos).pack(side="left", padx=(0, 8))
        tk.Button(bloque_btns, text="Salir", font=("Segoe UI", 10), bg=COLOR_TOPBAR, fg="#E8F5E9",
                  activebackground=COLOR_TOPBAR2, activeforeground="white", bd=1, relief="solid", padx=14, pady=6,
                  cursor="hand2", command=self._salir).pack(side="left")
    ...
    def _build_form(self):
        ...
        campos = [("Tipo de Residuo", "entry_tipo", 0, 0, 1), ("Peso (kg)", "entry_peso", 0, 1, 1),
                  ("Fecha", "entry_fecha", 0, 2, 1)]
        for label_texto, attr, fila, col, colspan in campos:
            fila_label = fila * 2
            fila_entry = fila * 2 + 1
            tk.Label(frame, text=label_texto.upper(), font=("Segoe UI", 8, "bold"), bg=COLOR_FONDO,
                     fg=COLOR_LABEL).grid(row=fila_label, column=col, columnspan=colspan, sticky="w", padx=(0, 22),
                                          pady=(10, 2))
            entry = tk.Entry(frame, font=("Segoe UI", 10), bd=1, relief="solid", bg=COLOR_CAMPO_BG, fg=COLOR_TEXTO,
                             insertbackground=COLOR_TOPBAR, highlightthickness=1, highlightbackground=COLOR_BORDE,
    ...
    def _build_tabla(self):
        ...
        tk.Label(fila_enc, text="REGISTROS DE BASURA", font=("Segoe UI", 10, "bold"), bg=COLOR_FONDO,
                 fg=COLOR_TOPBAR).pack(side="left")

        style = ttk.Style()
        style.theme_use("default")
        style.configure("Basura.Treeview.Heading", font=("Segoe UI", 9, "bold"), background=COLOR_TOPBAR,
                        foreground=COLOR_TH_FG, relief="flat")
        style.configure("Basura.Treeview", font=("Segoe UI", 10), rowheight=30, background=COLOR_CAMPO_BG,
                        fieldbackground=COLOR_CAMPO_BG, foreground=COLOR_TEXTO)
    ...
    def _actualizar(self):
        ...
        self.entry_tipo.insert(0, val[0])
        self.entry_peso.delete(0, tk.END);
        self.entry_peso.insert(0, val[1])
        self.entry_fecha.delete(0, tk.END);
        self.entry_fecha.insert(0, val[2])
