...
class JornadaVistaModerna(tk.Frame):
    ...
    def _build_interface(self):
        ...
        # Botón 1: Registrar
        self.btn_registrar = tk.Button(frame_botones, text="Registrar Jornada", command=self.registrar_jornada, **estilo_botones)
        self.btn_registrar.grid(row=0, column=0, padx=10, pady=6)

        # Botón 2: Eliminar por ID
        self.btn_eliminar = tk.Button(frame_botones, text="Eliminar por ID", command=self.eliminar_jornada, **estilo_botones)
        self.btn_eliminar.grid(row=0, column=1, padx=10, pady=6)

        # Botón 3: Limpiar Campos
        self.btn_limpiar = tk.Button(frame_botones, text="Limpiar Campos", command=self.limpiar_campos, **estilo_botones)
    ...
    def eliminar_jornada(self):
        id_j = self.entries["ID Jornada:"].get().strip()
        if not id_j:
            messagebox.showwarning("Atención", "Escriba el ID de la jornada que desea eliminar en el campo 'ID Jornada:'.")
            return
