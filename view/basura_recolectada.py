...
class BasuraRecolectadaVista(tk.Frame):
    ...
    def _build_topbar(self):
        ...
        tk.Label(bloque_texto,
                 text="Basura Recolectada",
                 font=("Segoe UI", 13, "bold"),
                 bg=COLOR_TOPBAR, fg="#E8F5E9").pack(anchor="w")

        tk.Label(bloque_texto,
                 text="Sistema de monitoreo costero  ·  Modulo de residuos",
                 font=("Segoe UI", 9),
    ...
    def _build_form(self):
        ...
        campos = [
            ("Tipo de Residuo", "entry_tipo",  0, 0, 1),
            ("Peso (kg)",       "entry_peso",  0, 1, 1),
    ...
    def _registrar(self):
        try:
            ...
            if not (tipo and peso and fecha):
            ...
            messagebox.showinfo("Exito",
                                f"Residuo '{tipo}' registrado ({peso} kg).")
    ...
    def _actualizar(self):
        ...
        valores = item["values"]
        self.entry_tipo.delete(0, tk.END);  self.entry_tipo.insert(0, valores[0])
        self.entry_peso.delete(0, tk.END);  self.entry_peso.insert(0, valores[1])
        self.entry_fecha.delete(0, tk.END); self.entry_fecha.insert(0, valores[2])
        ...
        if self.controller:
            ...
            for i, b in enumerate(registros):
                ...
                self.tabla.insert("", "end",
                                   values=(b.tipoResiduo,
                                           b.pesoKilos,
                                           b.fecha),
                                   tags=(tag,))
            self.lbl_footer.config(
                text=f"Mostrando {len(registros)} registro(s)")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Sistema Costero - Basura Recolectada")
    root.geometry("1000x650")
    root.resizable(False, False)

    class ControladorFalso:
        def registrar_basura(self, *a): print("[BASURA]", a)
        def get_all_basura(self): return []
        def volver_inicio(self): print("[SALIR]")

    BasuraRecolectadaVista(root, ControladorFalso())
    root.mainloop()