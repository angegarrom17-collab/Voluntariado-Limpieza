...
class FaunaVista(tk.Frame):
    ...
    def _build_topbar(self):
        ...
        tk.Label(bloque_texto,
                 ...
                 bg=COLOR_TOPBAR, fg="#E0F2F1").pack(anchor="w")

        tk.Label(bloque_texto,
                 text="Sistema de monitoreo costero  ·  Modulo de fauna marina",
                 font=("Segoe UI", 9),
                 bg=COLOR_TOPBAR, fg="#B2DFDB").pack(anchor="w")
    ...
    def _build_form(self):
        ...
        campos = [
            ("ID Animal",   "entry_id",          0, 0, 1),
            ("Especie",     "entry_especie",      0, 1, 1),
            ("Estado",      "entry_estado",       0, 2, 1),
            ("Descripcion", "entry_descripcion",  1, 0, 3),
        ]
    ...
    def _registrar(self):
        try:
            ...
            messagebox.showinfo("Exito", f"Animal '{esp}' registrado.")
        ...
        if self.controller:
            try:
                ...
                if animal:
                    ...
                    self.entry_especie.insert(0, animal.especie)
                    self.entry_estado.delete(0, tk.END)
                    self.entry_estado.insert(0, animal.estado)
                    self.entry_descripcion.delete(0, tk.END)
                    self.entry_descripcion.insert(0, animal.descripcion)
                else:
                    messagebox.showinfo("Sin resultados",
                                        f"No se encontro animal con ID '{id_a}'.")
            except Exception as e:
                messagebox.showerror("Error", str(e))

    def _limpiar_campos(self):
        for attr in ("entry_id", "entry_especie",
                     "entry_estado", "entry_descripcion"):
        ...
        if self.controller:
            ...
            for i, a in enumerate(animales):
                ...
                self.tabla.insert("", "end",
                                   values=(getattr(a, "idAnimal", ""),
                                           a.especie, a.estado,
                                           a.descripcion),
                                   tags=(tag,))
            self.lbl_footer.config(
                text=f"Mostrando {len(animales)} animal(es) registrado(s)")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Sistema Costero - Fauna Afectada")
    root.geometry("1000x650")
    root.resizable(False, False)

    class ControladorFalso:
        def registrar_animal(self, *a): print("[FAUNA]", a)
        def get_all_animales(self): return []
        def buscar_animal(self, id_): return None
        def volver_inicio(self): print("[SALIR]")

    FaunaVista(root, ControladorFalso())
    root.mainloop()