...
class MaterialVista(tk.Frame):
    ...
    def _build_topbar(self):
        ...
        tk.Label(bloque_texto,
                 text="Gestion de Materiales",
                 font=("Segoe UI", 13, "bold"),
                 bg=COLOR_TOPBAR, fg="#ECEFF1").pack(anchor="w")

        tk.Label(bloque_texto,
                 text="Sistema de voluntariado  ·  Modulo de inventario",
                 font=("Segoe UI", 9),
                 bg=COLOR_TOPBAR, fg="#CFD8DC").pack(anchor="w")
        ...
        tk.Button(bloque_btns,
                  ...
                  activebackground=COLOR_TOPBAR2, activeforeground="white",
                  bd=1, relief="solid", padx=14, pady=6, cursor="hand2",
                  command=self._buscar).pack(side="left", padx=(0, 8))
        ...
        tk.Button(bloque_btns,
                  ...
                  activebackground=COLOR_TOPBAR2, activeforeground="white",
                  bd=1, relief="solid", padx=14, pady=6, cursor="hand2",
                  command=self._limpiar_campos).pack(side="left", padx=(0, 8))
    ...
    def _build_form(self):
        ...
        campos = [
            ("ID Material",       "entry_id",       0, 0, 1),
            ("Nombre",            "entry_nombre",   0, 1, 1),
            ("Unidad de Medida",  "entry_unidad",   0, 2, 1),
            ("Cantidad Disponible","entry_cantidad", 0, 3, 1),
        ]
    ...
    def _registrar(self):
        try:
            ...
            if not (id_m and nom and uni and cant):
    ...
    def _usar_material(self):
        id_m = self.entry_id.get().strip()
        cant = self.entry_cantidad.get().strip()
        if not (id_m and cant):
            messagebox.showwarning("Atencion",
                                   "Ingrese el ID del material y la cantidad a usar.")
            return
        try:
            if self.controller:
                self.controller.usar_material(id_m, int(cant))
                messagebox.showinfo("Exito",
                                    f"Se usaron {cant} unidades del material '{id_m}'.")
                self._cargar_tabla()
        except ValueError:
            messagebox.showerror("Error", "La cantidad debe ser un numero entero.")
        except Exception as e:
        ...
        if self.controller:
            try:
                ...
                if material:
                    ...
                    self.entry_nombre.insert(0, material.nombre)
                    self.entry_unidad.delete(0, tk.END)
                    self.entry_unidad.insert(0, material.unidadMedida)
                    self.entry_cantidad.delete(0, tk.END)
                    self.entry_cantidad.insert(0, material.cantidadDisponible)
                else:
                    messagebox.showinfo("Sin resultados",
                                        f"No se encontro material con ID '{id_m}'.")
            except Exception as e:
                messagebox.showerror("Error", str(e))

    def _limpiar_campos(self):
        for attr in ("entry_id", "entry_nombre",
                     "entry_unidad", "entry_cantidad"):
        ...
        if self.controller:
            ...
            for i, m in enumerate(materiales):
                ...
                self.tabla.insert("", "end",
                                   values=(m.idMaterial, m.nombre,
                                           m.unidadMedida,
                                           m.cantidadDisponible),
                                   tags=(tag,))
            self.lbl_footer.config(
                text=f"Mostrando {len(materiales)} material(es) en inventario")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Sistema Costero - Materiales")
    root.geometry("1000x650")
    root.resizable(False, False)

    class ControladorFalso:
        def registrar_material(self, *a): print("[MATERIAL]", a)
        def get_all_materiales(self): return []
        def buscar_material(self, id_): return None
        def usar_material(self, *a): print("[USAR]", a)
        def volver_inicio(self): print("[SALIR]")

    MaterialVista(root, ControladorFalso())
    root.mainloop()